...
function Status({ children, tone = "teal" }: { children: React.ReactNode; tone?: "teal" | "green" | "purple" | "red" }) {
  const colors = {
  ...
  return (
    <article className="rounded-[var(--radius-md)] border border-[var(--border)] bg-[var(--card)] p-4 shadow-[var(--shadow-sm)]">
      <div className="mb-3 flex items-start justify-between gap-3">
        <div>
          <p className="mb-1 text-[11px] font-extrabold uppercase tracking-[.08em] text-[var(--primary)]">Próximo curso</p>
...
function SectionTitle({ title, action }: { title: string; action?: string }) { return <div className="mb-3 flex items-center justify-between gap-3"><h2 className="font-heading text-[16px] font-bold">{title}</h2>{action && <a href="#" className="text-[11px] font-extrabold text-[var(--accent)]">{action}</a>}</div>; }
